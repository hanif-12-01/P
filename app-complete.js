...
function initApp() {
    ...
    // Fix map rendering after DOM is ready
    setTimeout(() => {
        if (state.map) {
...
async function loginWithGoogle() {
    // Real Google OAuth with Firebase Auth
    if (!window.firebaseAuth || !window.GoogleAuthProvider || !window.signInWithPopup) {
        // Fallback to simulation if Firebase not configured
    ...
    try {
        ...
        updateAuthUI();
        closeLoginModal();
        showToast(`Selamat datang, ${state.currentUser.name}!`);
    } catch (error) {
...
function updateAuthUI() {
    ...
    console.log('updateAuthUI called', authBtn, state.currentUser);
    ...
    if (state.currentUser) {
        const photoHtml = state.currentUser.photo 
            ? `<img src="${state.currentUser.photo}" alt="Profile" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;">` 
        ...
        `;
        if (adminNav && state.currentUser.role === 'admin') {
            adminNav.style.display = 'block';
        }
    } else {
        authBtn.innerHTML = `
            <button onclick="showLoginModal()" class="btn btn-login">
